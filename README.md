...
1. Text instruction: t / role name / dialogue text / voice / delay index: delay time /. / (variable length). 
Exp: T/Jack/Hello/Jack01.mp3/2:0.5 (delay 0.5s when playing to e). 
Try not to delay continuously, for example, T/Ketty/12345//2:0.5/3:0.5/4:0.5 can be changed to = > T/Ketty/12345//2:0.5/ 4, 5, 0. 5, 6, 0. 5, 0. 5 /. 

two。. System instruction: C / character img/img position / x offset / y offset / background img/ background audio/. 
The default is 0, which distinguishes the positive from the negative. 
Img position can only be filled in 0 12, 0-left, 1-center, 2-right. 
...
Exp: A/ch_shake/. 
For custom animation, please override the Duo1JAnimation interface and inject it into AVGFrame through the property accessor. 

!!! All instructions end with /, and / / is required if a parameter is left empty. 
Exp: Tmax MaryCue GOOBYYAGUBYAGUBY / voice and delay are not used. 
